...
const Main = () => {
  ...
  const [revenueProjection, setRevenueProjection] = useState([30, 45, 57, 75, 85, 95])
    ...
    datasets: [
      {
        label: 'Your Business',
        data: [
          Math.round((businessMetrics.marketPotential + businessMetrics.scalability) / 2),
  ...
  const analyzeAIResponse = (response) => {
    ...
    try {
      ...
      const industryKeywords = {
        tech: ['tech', 'software', 'digital', 'technology', 'it ', 'app', 'saas', 'platform', 'online', 'internet', 'web', 'cloud', 'ai', 'machine learning', 'blockchain'],
        food: ['food', 'restaurant', 'catering', 'culinary', 'bakery', 'meal', 'cooking', 'kitchen', 'chef', 'dining', 'cafe', 'grocery', 'menu'],
        retail: ['retail', 'store', 'shop', 'e-commerce', 'ecommerce', 'commerce', 'merchandise', 'mall', 'shopping', 'consumer goods', 'products', 'sales'],
        healthcare: ['healthcare', 'medical', 'health', 'wellness', 'clinic', 'hospital', 'doctor', 'patient', 'therapy', 'pharma', 'medicine', 'treatment', 'care']
      };
      ...
      const industryScores = Object.entries(industryKeywords).reduce((scores, [industry, keywords]) => {
      ...
      const extractDataPoint = (metricName, defaultValue, min = 0, max = 100) => {
        ...
        const qualitativePatterns = {
          high: ['high', 'strong', 'significant', 'substantial', 'considerable', 'excellent'],
          medium: ['medium', 'moderate', 'average', 'fair', 'reasonable', 'decent'],
        ...
        if (metricContext) {
          if (qualitativePatterns.high.some(term => metricContext.includes(term))) {
            return Math.floor(Math.random() * 15) + 75; // 75-90 range for high
          } else if (qualitativePatterns.medium.some(term => metricContext.includes(term))) {
            return Math.floor(Math.random() * 20) + 50; // 50-70 range for medium
          } else if (qualitativePatterns.low.some(term => metricContext.includes(term))) {
      ...
      newMetrics.marketPotential = extractDataPoint('market potential', 
        text.includes('high demand') || text.includes('growing market') || text.includes('large market') ? 85 :
        text.includes('moderate demand') || text.includes('stable market') ? 65 :
        text.includes('saturated market') || text.includes('limited demand') || text.includes('small market') ? 45 : 
        newMetrics.marketPotential
      );

      newMetrics.competitionLevel = extractDataPoint('competition', 
        text.includes('high competition') || text.includes('competitive market') || text.includes('saturated') ? 80 :
        text.includes('moderate competition') || text.includes('some competitors') ? 60 :
        text.includes('low competition') || text.includes('unique') || text.includes('few competitors') ? 40 : 
        newMetrics.competitionLevel
      );

      newMetrics.initialInvestment = extractDataPoint('investment', 
        text.includes('high investment') || text.includes('significant capital') || text.includes('expensive') ? 80 :
        text.includes('moderate investment') || text.includes('reasonable capital') ? 60 :
        text.includes('low investment') || text.includes('minimal capital') || text.includes('affordable') ? 40 : 
        newMetrics.initialInvestment
      );

      newMetrics.profitMargin = extractDataPoint('profit margin', 
        text.includes('high profit') || text.includes('profitable') || text.includes('high margin') ? 75 :
        text.includes('moderate profit') || text.includes('average margin') ? 55 :
        text.includes('low profit') || text.includes('slim margin') || text.includes('thin margin') ? 35 : 
        newMetrics.profitMargin
      );

      newMetrics.riskFactor = extractDataPoint('risk', 
        text.includes('high risk') || text.includes('risky') || text.includes('volatile') ? 80 :
        text.includes('moderate risk') || text.includes('manageable risk') ? 60 :
        text.includes('low risk') || text.includes('safe') || text.includes('stable') ? 40 : 
        newMetrics.riskFactor
      );

      newMetrics.scalability = extractDataPoint('scalability', 
        text.includes('highly scalable') || text.includes('great scaling') || text.includes('easy to scale') ? 85 :
        text.includes('moderately scalable') || text.includes('some scaling') ? 65 :
        text.includes('limited scalability') || text.includes('difficult to scale') || text.includes('not scalable') ? 45 : 
        newMetrics.scalability
      );

      newMetrics.innovationScore = extractDataPoint('innovation', 
        text.includes('highly innovative') || text.includes('novel') || text.includes('breakthrough') ? 85 :
        text.includes('moderately innovative') || text.includes('some innovation') ? 65 :
        text.includes('not innovative') || text.includes('common') || text.includes('standard') ? 45 : 
        newMetrics.innovationScore
      );

      newMetrics.marketTrend = extractDataPoint('market trend', 
        text.includes('trending') || text.includes('emerging market') || text.includes('growing trend') ? 85 :
        text.includes('stable trend') || text.includes('established market') ? 65 :
        text.includes('declining') || text.includes('shrinking market') || text.includes('fading trend') ? 35 : 
        newMetrics.marketTrend
      );

      newMetrics.customerDemand = extractDataPoint('customer demand', 
        text.includes('high demand') || text.includes('strong need') || text.includes('eager customers') ? 85 :
        text.includes('moderate demand') || text.includes('some interest') ? 65 :
        text.includes('low demand') || text.includes('little interest') || text.includes('niche market') ? 45 : 
        newMetrics.customerDemand
      );

      newMetrics.regulatoryRisk = extractDataPoint('regulatory', 
        text.includes('heavily regulated') || text.includes('strict compliance') || text.includes('legal challenges') ? 85 :
        text.includes('moderately regulated') || text.includes('some compliance') ? 65 :
        text.includes('lightly regulated') || text.includes('minimal compliance') || text.includes('few regulations') ? 35 : 
      ...
      if (mentionedGrowthRate === null) {
        ...
        mentionedGrowthRate = ((newMetrics.scalability / 100) * 0.4 + 
      ...
      if (baseRevenue === null) {
        ...
        baseRevenue = Math.round(30 + (marketFactor * 50) + (profitFactor * 30)) * industryMultiplier;
      }
      ...
      const newRevenue = months.map((month, i) => {
        ...
        const growthFactor = Math.pow(1 + (mentionedGrowthRate * (1 - (i * 0.05))), growthPeriods);
        
        // Apply seasonal fluctuations based on month number
        const seasonalFactor = 1 + (Math.sin(month / 3 * Math.PI) * 0.05);
        
        return Math.round(baseRevenue * growthFactor * randomFactor * seasonalFactor);
      });
      ...
      const successRate = Math.round((newMetrics.marketPotential * 0.3 + 
  ...
  const formatAIResponse = (response) => {
    ...
    let formattedResponse = response.replace(/(\d+)%/g, '<span class="highlight-percent">$1%</span>');
    
    // Highlight key metrics
    const keyMetrics = [
      'Market Potential', 'Competition Level', 'Investment', 'Profit Margin', 
      'Risk', 'Scalability', 'Innovation', 'Market Trend', 'Customer Demand', 
      'Regulatory', 'Growth Rate', 'Revenue'
    ];
    
    keyMetrics.forEach(metric => {
      const regex = new RegExp(`(${metric}[^.]*?)`, 'gi');
      formattedResponse = formattedResponse.replace(regex, '<span class="highlight-metric">$1</span>');
    });
  ...
  useEffect(() => {
    ...
    const updatedIndustryComparisonData = {
      labels: ['Success Rate', 'Growth Potential', 'Profit Margin', 'Market Position', 'Risk Assessment'],
      datasets: [
        {
          label: 'Your Business',
          data: [
        ...
        },
        {
          ...
          data: [
            ...
            industryType === 'tech' ? 55 : 
            industryType === 'healthcare' ? 70 :
            industryType === 'food' ? 60 : 50 // Risk assessment by industry
          ],
          backgroundColor: 'rgba(255, 99, 132, 0.7)'
        }
      ]
  ...
  const getLineChartData = () => {
    try {
      ...
      const yMean = validRevenue.reduce((sum, val) => sum + val, 0) / validRevenue.length;
      ...
      const trendLine = xValues.map(x => Math.max(0, Math.round(x * slope + intercept)));
      ...
      return {
        labels: ['Month 1', 'Month 3', 'Month 6', 'Month 9', 'Month 12', 'Month 15'],
        datasets: [
          {
  ...
  const getFeasibilityData = () => {
    try {
      ...
      const benchmark = INDUSTRY_BENCHMARKS[industryType] || INDUSTRY_BENCHMARKS.tech;
      ...
      const safeMetrics = Object.keys(businessMetrics).reduce((safe, key) => {
        ...
        safe[key] = isNaN(value) ? defaultBusinessMetrics[key] : 
      ...
      const benchmarkValues = {
        ...
        initialInvestment: benchmark.initialInvestment === 'High' ? 80 : 
                          benchmark.initialInvestment === 'Medium' ? 60 : 40,
        profitMargin: benchmark.avgProfitMargin,
        riskFactor: benchmark.breakEvenMonths > 24 ? 70 : 
                   benchmark.breakEvenMonths > 12 ? 55 : 40,
        scalability: benchmark.avgGrowthRate > 20 ? 75 : 60,
        innovationScore: industryType === 'tech' ? 80 : 
                        industryType === 'healthcare' ? 70 : 60,
        marketTrend: benchmark.marketSize === 'Large' ? 75 : 
                    benchmark.marketSize === 'Medium' ? 60 : 45,
        customerDemand: benchmark.avgGrowthRate > 15 ? 70 : 55,
        regulatoryRisk: industryType === 'healthcare' ? 80 : 
                       industryType === 'food' ? 65 : 
                       industryType === 'tech' ? 45 : 55
      };
      ...
      <div className='main-container'>
        {!showResult ? (
          <>
            <div className='greet'>
              ...
              <p>Share your business idea for instant feasibility analysis</p>
              <div className='welcome-hint'>
                <p>Enter your business concept in the field below and press Enter or click the send button to get a detailed analysis.</p>
                <p className="charts-info">After submitting, interactive charts and AI-powered analysis will appear here.</p>
              </div>
            </div>
            {/* No graphs shown on welcome page */}
          </>
        ) : (
              ...
              {loading ? (
                ...
                <>
                  <div className='error-container'>
                    ...
                    <p>{error}</p>
                    <p>Please try again with a different business idea or check your connection.</p>
                  </div>
                </>
              ) : (
                <>
                  <div className='statistics-dashboard'>
                    ...
                    <div className='industry-info'>
                      <h3>Industry: {industryType.charAt(0).toUpperCase() + industryType.slice(1)}</h3>
                      <p>Average Break-even: {INDUSTRY_BENCHMARKS[industryType].breakEvenMonths} months</p>
                    </div>
                    ...
                    <div className='gemini-response'>
                      ...
                      <p dangerouslySetInnerHTML={{ __html: formatAIResponse(resultData) }}></p>
                    </div>
                  </div>
                </>
              )}
        ...
        <div className='main-bottom'>
          ...
          <p className='bottom-info'>
            Get real-time feasibility analysis, market insights, and growth projections for your business idea.
          </p>
        </div>
      </div>
    </div>
